...
import { getNonTechJob } from '../../../services/api';

const NonTechJobDetails = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchJob = async () => {
      try {
        const data = await getNonTechJob(id);
        setJob(data);
      } catch (error) {
        console.error('Error fetching non-tech job:', error);
      } finally {
        setLoading(false);
      }
    };
...
export default NonTechJobDetails;
